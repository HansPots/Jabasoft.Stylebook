...
public static class MonacoDiffHtml
{
    public const string Content = """
        ...
        <script>
            ...
            // door de letterlijke waarde van het aangeklikte token - of
            // voegt 'm in op de cursorpositie als er niets geselecteerd is
            // (een "selectie" van lengte 0 is in Monaco gewoon een geldige
            // range). Altijd een kale waarde, nooit een
            // {DynamicResource ...}-verwijzing: dat voorkomt precies het
            // type-mismatch-probleem waar de vroegere Parts-aanpak voor
            // nodig was (WPF staat maar één markup-extension per
            // attribuutwaarde toe, dus een token combineren met losse
            // cijfers in dezelfde CornerRadius/Margin/Padding-waarde kon
            // nooit werken zonder de hele boel naar Parts-attributen te
            // herschrijven) - een letterlijk getal mixt daarentegen gewoon
            // vrij in zo'n komma-lijst, dus is die hele herschrijf-stap
            // niet meer nodig. executeEdits triggert modifiedModel's eigen
            // onDidChangeContent hierboven vanzelf, dus dit synchroniseert
            // automatisch terug naar _proposedXaml zoals elke andere
            // wijziging.
            window.replaceSelectionWithValue = function (value) {
                if (!diffEditor) { return; }
                var modifiedEditor = diffEditor.getModifiedEditor();
                var selection = modifiedEditor.getSelection();
                modifiedEditor.executeEdits("replace-with-value", [
